...
build_env="WEREWOLF_RELEASE_VERSION=$RELEASE_VERSION WEREWOLF_GIT_COMMIT=$commit SOURCE_DATE_EPOCH=$epoch"
(cd "$ROOT" && env $build_env cargo build --locked --offline --release --workspace) || fail 'locked offline release build failed'
