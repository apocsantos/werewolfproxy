...
validate_shape() {
  ...
  if jq -S 'paths | map(if type == "number" then "[]" else tostring end) | join(".")' "$contract" >/tmp/schema-ref.txt \
    && jq -S 'paths | map(if type == "number" then "[]" else tostring end) | join(".")' /tmp/schema-current.json >/tmp/schema-current.txt; then
